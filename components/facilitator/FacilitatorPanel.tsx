...
const PARAM_GROUPS: { label: string; keys: NumericParamKey[] }[] = [
  {
    label: 'Crecimiento y poblacion',
    keys: [
  ...
  {
    label: 'Presiones politicas — disipacion',
    keys: [
...
const FacilitatorPanel: React.FC<FacilitatorPanelProps> = ({ controlParams, onChange, onClose }) => {
  const handleChange = (key: NumericParamKey, value: string) => {
    ...
    if (!isNaN(parsed)) {
      onChange({ ...controlParams, [key]: parsed });
    }
  };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="facilitator-panel-title"
    >
      <div className="bg-custom-light-gray p-6 sm:p-8 rounded-lg shadow-2xl max-w-2xl w-full text-gray-100 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <h2 id="facilitator-panel-title" className="text-2xl font-bold text-custom-accent">
        ...
        <p className="text-xs text-yellow-400 bg-yellow-900 bg-opacity-40 rounded px-3 py-2 mb-4">
          Atencion: los cambios aqui afectan la simulacion en tiempo real. Usar solo con fines de facilitacion de talleres.
        </p>

        <div className="overflow-y-auto flex-1 space-y-6 pr-1">
          {PARAM_GROUPS.map(group => (
            <div key={group.label}>
              <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3 border-b border-gray-600 pb-1">
                {group.label}
              </h3>
        ...
        <button
          onClick={onClose}
          className="mt-4 w-full px-4 py-2 bg-custom-accent hover:bg-blue-600 text-white font-semibold rounded-lg transition-colors text-sm"
        >
          Cerrar
        </button>
      </div>
