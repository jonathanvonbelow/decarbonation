...
import type { PostSurveyData } from './components/surveys/SurveyPost';
...
export const App = () => {
  ...
  // ControlParams elevado a estado React para el FacilitatorPanel
  ...
  // Mantener controlParamsRef sincronizado con el estado
  ...
  useEffect(() => {
    ...
    // Mostrar encuesta pre-juego para usuarios autenticados (solo una vez por sesión,
    // y solo cuando no hay otro modal activo)
    try {
      if (
        authStage === 'authenticated' &&
        !sessionStorage.getItem('decarbonationPreSurveyDone_v1') &&
        !showTutorialModal &&
        !showLevelIntroModalForLevel &&
    ...
    // Iniciar sesión en Supabase al montar
  ...
  useEffect(() => {
    if (gameState.gameOverReason && !hasSentFinalDecarbonitoMessage) {
      ...
      // Mostrar encuesta post-juego solo si el abandon no la abrió ya
      // (el abandon ya establece 'abandono' y showPostSurvey=true antes de setear gameOverReason)
      const isAbandon = gameState.gameOverReason.includes('abandonada');
      if (!isAbandon) {
        const resultado = gameState.gameOverReason.toLowerCase().includes('victoria') ? 'victoria' : 'derrota';
        setPostSurveyResult(resultado as 'victoria' | 'derrota' | 'abandono');
...
// FIX: Explicitly typed 'inst' as PolicyInstrument to resolve type inference issues where Object.values().reduce() was inferring 'inst' as 'unknown', causing property access errors.
  ...
  const runSimulationRound = useCallback(async () => {
    // Usar ref para acceder a los parámetros de control actualizados por el FacilitatorPanel
...
// FIX: Add explicit type annotation for 'p' in the find method to resolve 'unknown' type errors on property access.
  ...
  const handleAbandonGame = useCallback(() => {
    if (gameState.gameOverReason) return; // ya terminó
    setPostSurveyResult('abandono');
    setShowPostSurvey(true);
    setGameState(s => ({ ...s, gameOverReason: 'Partida abandonada por el jugador.' }));
  }, [gameState.gameOverReason]);

  // Gates de autenticación — deben ir ANTES del return principal
    ...
    <div className="bg-custom-gray min-h-screen text-gray-200 font-sans">
      ...
      {authStage === 'demo' && (
        <div className="bg-yellow-900 text-yellow-200 text-xs text-center py-1 px-4">
          Jugando en modo demo &mdash; tus datos no se guardan.{' '}
          <button onClick={handleSignOut} className="underline hover:text-yellow-100">Iniciar sesion</button> para guardarlos.
        </div>
      )}
      ...
      {showPreSurvey && (
        <SurveyPre
          onComplete={(data: PreSurveyData) => {
            setShowPreSurvey(false);
            try { sessionStorage.setItem('decarbonationPreSurveyDone_v1', '1'); } catch(_) {}
            savePreSurvey(data);
          }}
          onSkip={() => {
            setShowPreSurvey(false);
      ...
      {showPostSurvey && (
        <SurveyPost
          ...
          onComplete={(data: PostSurveyData) => {
      ...
      {showGameSummary && (
        <GameSummaryPanel
          ...
          onPlayAgain={() => {
            ...
            try { sessionStorage.removeItem('decarbonationPreSurveyDone_v1'); } catch(_) {}
          }}
        />
      )}
